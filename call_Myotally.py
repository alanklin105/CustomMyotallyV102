        ...
        if path == folder:
            ...
            for d in direcs:
                ...
                if d != '.DS_Store' and len(set(keep_files)&set(os.listdir(folder+'/'+d))) == len(set(keep_files)):
                    ...
                    mode = int(st.mode(I1.flatten(), keepdims=True).mode)
                    ...
                    base_name = os.path.splitext(start_Lam)[0]
                    final_output_name = f"{base_name}_final.tif"

                    I = Image.open('final_blank.tif')
                    write = ImageDraw.Draw(I)
                    CSAs = []
                    for f in fibers:
                        write.text((f.centroid), str(f.number), fill='white')
                        CSAs.append(f.area)

                    # Save with the dynamic name
                    I.save(final_output_name)
                    keep_files.append(final_output_name)
                    del write
